...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  public appHealthiness: 'Healthy' | 'Unhealthy' = 'Unhealthy';
  public appVersion = '';
  ...
  getControlPlaneData(): void {
    this.instanceService.getControlPlaneStatus().subscribe((data: Status[]) => {
      this.appHealthiness = data.every((service) => service.healthy === 'True') ? 'Healthy' : 'Unhealthy';
      if (data.length === 0) {
        this.appHealthiness = 'Unhealthy';
      }
      data.forEach(service => {
        this.appVersion = service.version;
      });
  ...
  stop(id: string): void {
    if (confirm('You are about to stop Bhojpur Application instance with ID "' + id + '". Confirm?')) {
      this.instanceService.stopInstance(id).subscribe(() => {
        this.showSnackbar('Stopped Bhojpur Application instance with ID ' + id);
        this.getInstances();
      }, error => {
        this.showSnackbar('Failed to stop Bhojpur Application instance with ID ' + id);
      });
    }
  }
