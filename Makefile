...
# Go build details                                                             #
################################################################################
BASE_PACKAGE_NAME := github.com/bhojpur/dashboard
...
APP_ARTIFACTS := $(OUT_DIR)/artifacts
APP_OUT_DIR := $(OUT_DIR)/$(GOOS)_$(GOARCH)
APP_LINUX_OUT_DIR := $(OUT_DIR)/linux_$(GOARCH)
...
# Generate builds for Bhojpur Application binaries for the target
# Params:
...
ifeq ($(GOOS),windows)
dashboard_$(1).zip:
	7z.exe a -tzip "$(ARCHIVE_OUT_DIR)/$(1)_$(GOOS)_$(GOARCH).zip" "$(APP_OUT_DIR)"
else
dashboard_$(1).zip:
	zip -r -q "$(ARCHIVE_OUT_DIR)/$(1)_$(GOOS)_$(GOARCH).zip" "$(APP_OUT_DIR)"
endif
dashboard_$(1).tar.gz:
	tar -zcf "$(ARCHIVE_OUT_DIR)/$(1)_$(GOOS)_$(GOARCH).tar.gz" "$(APP_OUT_DIR)"
endef
...
run-backend-standalone:
	$(APP_OUT_DIR)/dashboard$(BINARY_EXT)

run-backend-kubernetes:
	APP_DASHBOARD_KUBECONFIG=~/.kube/config $(APP_OUT_DIR)/dashboard$(BINARY_EXT)